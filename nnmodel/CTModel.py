...
class CTModel(BaseNNModel):
    ...
    def predict(self,
        ...
        if self.model_segmentation is not None:
            ...
            if self.np_mask and self.detected_roi:
                print("Область с образованием успешно выделена")

            if result_dir is not None and save_detection_video == True:
                print("Видео с областью детекции успешно сохранено в папку", result_dir)

            if result_dir is not None and save_segmentation_video == True:
                print("Видео с областью сегментации успешно сохранено в папку", result_dir)
        ...
        if self.model_classification is not None and self.np_mask is not None and self.np_video is not None:
            ...
            if self.video_with_mask:
                print("Образование успешно выделено маской на видео")

            self.proba, self.label, self.label_name = self.model_classification.predict(self.video_with_mask)

            if self.proba and self.label and self.label_name:
                print("Модель успешно предсказала класс образования")
                print("Предсказание класса образования:", self.label_name)